...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        if self.args.beat_align:
            ...
            self.avg_vel = np.load(args.data_path+f"weights/mean_vel_{args.pose_rep}.npy")     # [55,  ]  平均速度
        ...
        self.norm = True
        self.mean = np.load(f'/sata/public/lixr/EmoDiffGes/mean_std/beatx_combined_pose_mean.npy')
        self.std = np.load(f'/sata/public/lixr/EmoDiffGes/mean_std/beatx_combined_pose_std.npy')

        
        self.trans_mean = np.load(f'/sata/public/lixr/EmoDiffGes/mean_std/beatx_combined_trans_mean.npy')
        self.trans_std = np.load(f'/sata/public/lixr/EmoDiffGes/mean_std/beatx_combined_trans_std.npy')
    ...
    def calculate_mean_velocity(self, save_path):
        ...
        for tar in tqdm(os.listdir(dir_p)):
            if tar.endswith(".npz") and int(tar.split("_")[0]) in self.args.training_speakers:
                ...
                for i in range(s):
                    with torch.no_grad():
                        joints = self.smplx(
                            ...
                            jaw_pose=poses[i*max_length:(i+1)*max_length, 66:69],       #下颚关节旋转参数
                            global_orient=poses[i*max_length:(i+1)*max_length,:3],      #全局旋转参数
                            body_pose=poses[i*max_length:(i+1)*max_length,3:21*3+3],     #身体关节参数，21个关节，每个关节3个值，共63个
                    ...
                    all_tensor.append(joints)         # [s, 128, 165]
                if r != 0:
                    with torch.no_grad():
                        joints = self.smplx(
                    ...
                    all_tensor.append(joints)     # 最后一维 [r,165]
                joints = torch.cat(all_tensor, axis=0)        # [frame, 165]
            ...
            # first steps is forward diff (t+1 - t) / dt
                ...
                vel_seq = torch.cat([init_vel, middle_vel, final_vel], dim=1).permute(1, 0).reshape(n, 55, 3)     # [frame, 55, 3]
                #print(vel_seq.shape)
    ...
    def build_cache(self, preloaded_dir):
        ...
        if os.path.exists(preloaded_dir):
            ...
            #self.calculate_pose_mean_std(preloaded_dir)
        else:
            self.cache_generation(
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        processed_frames = set()        #本地去重
        frame_count = 0              #本地帧数
        pose_sum = np.zeros(330, dtype=np.float32)
        pose_sum_sq = np.zeros(330, dtype=np.float32)
        trans_v_sum = np.zeros(3, dtype=np.float32)
        trans_v_sum_sq = np.zeros(3, dtype=np.float32)
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if "smplx" in self.args.pose_rep:
                ...
                pose_each_file = pose_data["poses"][::stride] * self.joint_mask        # [frame, 165]
                pose_each_file = pose_each_file[:, self.joint_mask.astype(bool)]
                
                self.beatx_during_time += pose_each_file.shape[0]/30
                trans_each_file = pose_data["trans"][::stride]       # [frame, 3]
                trans_each_file[:,0] = trans_each_file[:,0] - trans_each_file[0,0]      #将第一帧的x坐标变为0，每一帧都变为相对于第一帧的坐标
                trans_each_file[:,2] = trans_each_file[:,2] - trans_each_file[0,2]      #将第一帧的z坐标变为0
                trans_v_each_file = np.zeros_like(trans_each_file)
                trans_v_each_file[1:,0] = trans_each_file[1:,0] - trans_each_file[:-1,0]      #每一帧的x轴平移差，即速度
                trans_v_each_file[0,0] = trans_v_each_file[1,0]           #将第一帧的速度值设为第二帧的速度值，以防第一帧速度为0
                trans_v_each_file[1:,2] = trans_each_file[1:,2] - trans_each_file[:-1,2]       #每一帧的z轴平移差，即速度
                trans_v_each_file[0,2] = trans_v_each_file[1,2]            #将第一帧的速度值设为第二帧的速度值
                trans_v_each_file[:,1] = trans_each_file[:,1]
                
                
                shape_each_file = np.repeat(pose_data["betas"].reshape(1, 300), pose_each_file.shape[0], axis=0)       # [frame, 300]
                if self.args.facial_rep is not None:
                    logger.info(f"# ---- Building cache for Facial {id_pose} and Pose {id_pose} ---- #")
                    facial_each_file = pose_data["expressions"][::stride]           # [frame, 100]
            ...
            if self.args.id_rep is not None:
                vid_each_file = np.repeat(np.array(int(f_name.split("_")[0])-1).reshape(1, 1), pose_each_file.shape[0], axis=0)        # [frame, 1]
            ...
            filtered_result = self._sample_from_clip(
                ...
                id_pose, processed_frames, pose_sum, pose_sum_sq, trans_v_sum, trans_v_sum_sq, frame_count
                )
            frame_count = filtered_result["frame_count"] 
            for type in filtered_result["n_filtered_out"].keys():
                n_filtered_out[type] += filtered_result["n_filtered_out"][type]
        ...
        save_dir = "/sata/public/lixr/EmoDiffGes/mean_std/"
        os.makedirs(save_dir, exist_ok=True)
        stats = {
            "frame_count": frame_count,
            "pose_sum": pose_sum,
            "pose_sum_sq": pose_sum_sq,
            "trans_v_sum": trans_v_sum,
            "trans_v_sum_sq": trans_v_sum_sq
        }
        np.save(os.path.join(save_dir, f"beatx_{self.loader_type}_stats.npy"), stats)
    ...
    def _sample_from_clip(
        ...
        for ratio in self.args.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                sample_pose = pose_each_file[start_idx:fin_idx]         # [64,165]
                sample_trans = trans_each_file[start_idx:fin_idx]       # [64, 3]
                sample_trans_v = trans_v_each_file[start_idx:fin_idx]   # [64, 3]
                sample_shape = shape_each_file[start_idx:fin_idx]       # [64, 300]
                sample_face = facial_each_file[start_idx:fin_idx]       # [64, 100]
                ...
                if sample_pose.any() != None:
                    ...
                    if is_correct_motion or disable_filtering:
                        sample_pose_6d = torch.from_numpy(sample_pose).float()
                        sample_pose_6d = rc.axis_angle_to_matrix(sample_pose_6d.reshape(-1, 55, 3))
                        sample_pose_6d = rc.matrix_to_rotation_6d(sample_pose_6d).reshape(-1, 55*6).numpy()

                        for j in range(sample_pose_6d.shape[0]):
                            frame_idx = start_idx + j
                            frame_key = (id_pose, frame_idx)
                            if frame_key not in processed_frames:
                                ...
                                pose_sum_sq += x * x
                                # Trans_v 总值
                                x_v = sample_trans_v[j]
                                trans_v_sum += x_v
                                trans_v_sum_sq += x_v * x_v
        ...
        return {"n_filtered_out": n_filtered_out, "frame_count": frame_count}

    def calculate_pose_mean_std(self, lmdb_path):
        all_poses = []
        all_trans_v = []
        save_dir = "/sata/public/lixr/EmoDiffGes/mean_std/"
        pose_mean_path = os.path.join(save_dir, f"beatx_{self.loader_type}_pose_mean.npy")
        pose_std_path = os.path.join(save_dir, f"beatx_{self.loader_type}_pose_std.npy")
        trans_mean_path = os.path.join(save_dir, f"beatx_{self.loader_type}_trans_mean.npy")
        trans_std_path = os.path.join(save_dir, f"beatx_{self.loader_type}_trans_std.npy")

        # 打开 LMDB 数据库
        with lmdb.open(lmdb_path, readonly=True, lock=False) as lmdb_env:
            with lmdb_env.begin(write=False) as txn:
                for idx in tqdm(range(self.n_out_samples)):
                    ...
                    tar_pose, _, _, _, _, trans_v = sample
                    ...
                    # 收集数据
                    all_poses.append(tar_pose.numpy())
                    all_trans_v.append(trans_v)

        # 合并所有样本的 6D 数据
        all_poses = np.concatenate(all_poses, axis=0)  # [total_frames, 330]
        all_trans_v = np.concatenate(all_trans_v, axis = 0)

        # 计算均值和标准差
        pose_mean = np.mean(all_poses, axis=0)  # [330]
        pose_std = np.std(all_poses, axis=0)    # [330]

        trans_mean = np.mean(all_trans_v, axis=0)
        trans_std = np.std(all_trans_v, axis=0)

        np.save(pose_mean_path, pose_mean)
        np.save(pose_std_path, pose_std)
        np.save(trans_mean_path, trans_mean)
        np.save(trans_std_path, trans_std)
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if self.loader_type == "test":
                ...
                in_shape = torch.from_numpy(in_shape).reshape((in_shape.shape[0], -1)).float()     # [64,300]
                trans = torch.from_numpy(trans).reshape((trans.shape[0], -1)).float()          # [64,3]
                trans_v = torch.from_numpy(trans_v).reshape((trans_v.shape[0], -1)).float()    # [64,3]
                vid = torch.from_numpy(vid).reshape((vid.shape[0], -1)).float()          # [64,1]
                tar_pose = tar_pose.reshape((tar_pose.shape[0], -1)).float()     # [64,330]
                tar_pose = torch.cat([tar_pose, trans_v], dim=1)                 # [64,333]
                tar_pose = torch.cat([tar_pose, tar_face], dim=1)               # [64,433]
