...
class RVQVAE(nn.Module):
    ...
    def forward(self, x):
        x_in = self.preprocess(x)       # [bs,dim, 64]
        # Encode
        x_encoder = self.encoder(x_in)       # [bs, 128, 16]
        ...
        # print(code_idx[0, :, 1])
        ## decoder
        x_out = self.decoder(x_quantized)      # [bs,64,d]
    ...
    def map2latent(self,x):
        x_in = self.preprocess(x)       # [bs, 128, dim] -> [bs, dim, 128]
        # Encode
        x_encoder = self.encoder(x_in)       # [bs, 128, 32]
        x_encoder = x_encoder.permute(0,2,1)   # [bs, 32, 128]
        return x_encoder
