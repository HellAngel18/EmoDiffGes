...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        shortcut = x          # [bs, 2, 68266]    [bs, 64, 14331]   [bs, 64, 2387]    [bs, 128, 396]
        x = self.conv1(x)     # [bs, 64, 14331]   [bs, 64, 2387]    [bd, 128, 396]     [bs, 256, 128]
        x = self.bn1(x)       #[bs, 64, 14331]
        x = self.act1(x)      # [bs, 64, 14331]
        x = self.conv2(x)     # [bs, 64, 14331]
        x = self.bn2(x)       # [bs, 64, 14331]
        if self.downsample is not None:
