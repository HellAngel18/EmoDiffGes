...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k = nn.Linear(self.context_dim, dim, bias=qkv_bias)
        self.v = nn.Linear(self.context_dim, dim, bias=qkv_bias)
...
class JointAttention(nn.Module):
    def __init__(self, dim, num_heads=8, dropout=0.0, spatial_first=False, enable_spatial=True):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
...
class SpatialTemporalBlock(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_temporal_attn = JointAttention(dim, num_heads=num_heads, dropout=attn_drop, enable_spatial=enable_spatial)
        self.ls1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
